...
<div class='flex flex-col-reverse mx-auto max-w-[400px] text-center'>
    <form action="traitement_crud/link_acteur_traitement.php" method="POST" id='form_acteur'>
        <input type='submit'>
    </form>
    ...
    <?php
    $sql = "SELECT * FROM `acteurs` ORDER BY prenom_acteur";
    $acteur = $pdo->prepare($sql);
    $acteur -> execute();  
        while($acteurs = $acteur->fetch(PDO::FETCH_ASSOC, PDO::FETCH_ORI_NEXT)){
            echo"
                <option value='$acteurs[id_acteur]'>$acteurs[prenom_acteur] $acteurs[nom_acteur]</option>";
        }
            ?>
    </select>
